...
from lucid_dataset_parser import (process_live_traffic, 
                                  dataset_to_list_of_fragments,
                                  parse_labels)
from util_functions import (load_dataset, LUCID_TRAIN_SEED, static_min_max, 
                            count_packets_in_dataset, normalize_and_padding)
from global_constants import LucidModel

# Seed Random Numbers
os.environ['PYTHONHASHSEED']=str(LUCID_TRAIN_SEED)
np.random.seed(LUCID_TRAIN_SEED)
rn.seed(LUCID_TRAIN_SEED)


th.manual_seed(LUCID_TRAIN_SEED)
...
def main(argv):
    ...
    if args.train is not None:
        ...
        for full_path in subfolders:
            ...
            X_train, Y_train = shuffle(X_train, Y_train, random_state=LUCID_TRAIN_SEED)
            X_val, Y_val = shuffle(X_val, Y_val, random_state=LUCID_TRAIN_SEED)
            ...
            best_model = LucidModel()
            
            summary(best_model, input_size=(32,) + X_train.shape[1:])
            
            optim = th.optim.Adam(best_model.parameters(),
                                  lr = LEARNING_RATE,
                                  betas = (0.9, 0.999))
            loss_fn = nn.BCELoss()
            
            train_dataset = TensorDataset(*(th.as_tensor(X_train, dtype=th.float32), 
                                            th.as_tensor(Y_train.reshape(-1, 1), dtype=th.float32)))
            train_dataloader = DataLoader(dataset=train_dataset, batch_size=BATCH_SIZE)
            
            for _ in tqdm(range(DEFAULT_EPOCHS)):
            ...
            th.save(best_model.state_dict(), best_model_filename + ".pt")
            ...
            Y_pred_val = (best_model(th.as_tensor(X_val, dtype=th.float32)) > 0.5)
    ...
    if args.predict is not None:
        ...
        if args.model is not None:
            ...
            model_list = glob.glob(args.predict + "/*.pt")
        ...
        for model_path in model_list:
            ...
            if filename_prefix in filename:
                ...
                Y_pred = np.squeeze((model(th.as_tensor(X, dtype=th.float32)) > 0.5).detach().numpy())
    ...
    if args.predict_live is not None:
        ...
        if args.model is not None and args.model.endswith('.pt'):
        ...
        while (True):
            ...
            if len(samples) > 0:
                ...
                pt0 = time.time()
                Y_pred = np.squeeze((model(th.as_tensor(X, dtype=th.float32)) > 0.5).detach().numpy(), axis=1)
                pt1 = time.time()
